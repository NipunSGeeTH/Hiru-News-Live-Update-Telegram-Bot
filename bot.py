...
async def start_bot1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    existing_user = next((user for user in user_data if user.get("id") == user_id), None)
    ...
    if existing_user is None:
        ...
        with open('telegramID.json', 'w') as f:
            json.dump(user_data, f, indent=4)

    await update.message.reply_html(
        f"Hi {update.effective_user.mention_html()}! This isLive Hiru News Update  Bot . "
        f"All new News send you as a simple message and image"
...
async def send_news_updates(application, news):
    for news_item in news:
        ...
        try:
            ...
            main_image.save("a_with_logo.jpg")
