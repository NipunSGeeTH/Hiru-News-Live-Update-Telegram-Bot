...
def get_and_save_latest_news():
    with open("database/last_News_update_ID.json", "r") as f:
        last_update = json.load(f)
        last_id = int(last_update["ID"])

    # Load the news articles
    with open("database/hirunews_articles.json", "r", encoding="utf-8") as f:
        articles = json.load(f)

    # Find new articles
    new_articles = [
        article for article in articles if int(article["news_id"]) > last_id
    ]
    ...
    if new_articles:
        ...
        # Save the new articles to database/Latest_News.json
        with open(
            "database/Latest_News.json", "w", encoding="utf-8"
        ) as f:  # 'w' mode overwrites the file
        ...
        with open("database/last_News_update_ID.json", "w") as f:
            json.dump(last_update, f, indent=4)

        print(f"Updated last news update ID to {new_last_id}")
    else:
        print("No new articles found.")
        # Clear the contents of database/Latest_News.json
        with open("database/Latest_News.json", "w", encoding="utf-8") as f:
